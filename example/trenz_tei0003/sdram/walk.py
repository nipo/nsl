...
session, and anything else is the resource path.

The link is the board's own serial port, and it rides the 12 MHz
oscillator rather than the memory clock: the rack answers whatever the
PLL and the memory domain are doing, which is what lets a stalled walk
be read rather than guessed at.  What the bus did while each walk ran
is printed with it.  An error count says a walk went wrong and nothing
about where; the channel counters say which transfer a stalled walk is
waiting on, and the command counters say whether the controller is
still asking the part for anything at all.
"""
...
class Walk:
    ...
    TOLERANCE_PPM = 200
    POLL_SECONDS = 0.02
    # A whole-part walk at these rates is over in a few seconds, and a
    # walk that is not over in this long is one that will never end.
    TIMEOUT_SECONDS = 60.0
    REPORT_SECONDS = 1.0
    # The part's power-up sequence is two hundred microseconds and the
    # host cannot ask that fast; anything past this is a controller
    # that is not sequencing at all.
    READY_SECONDS = 5.0
    ...
    def __init__(self, path, count=1):
        ...
        self.count = count
        self.ram_hz = self.constant("ram_hz_c")
        self.capture_ck = self.constant("capture_ck_c")
        self.panel = None

    @staticmethod
    def constant(name):
        """One of the design's own constants.

        Read from the design rather than repeated here, so the two
        cannot drift while a figure is being swept.
        """
        source = pathlib.Path(__file__).with_name("src") / "boundary.vhd"
        found = re.search(rf"constant\s+{name}\s*:\s*natural\s*:=\s*(\d+)",
                          source.read_text())
        if not found:
    ...
    async def at_rest(self):
        ...
        host already believes is there does not reach the fabric.  One
        raise makes the drop a transition, and a transition is what
        resets anything.
        """
