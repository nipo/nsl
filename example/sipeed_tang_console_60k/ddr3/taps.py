...
comes through is finer.  Every data pin and both strobe pins sit behind
one, all stepped together by the panel's tick, and a tap on this family
is about 12.5 ps.
...
One thing the line does that a sweep has to allow for: the strobe,
sampled every slot when its own edges are a slot apart, resolves one
way for a whole burst rather than reading as a toggle, and which way it
resolves changes from pass to pass.  So the strobe word is taken
several times at each tap and the slots it was ever high in are what
the run is read from.

The line here counts up a tap per tick and wraps, and the mark is tap
zero, so a run of n ticks from the mark leaves the line at tap n.  It
carries 256 taps, which is two hundred and fifty-six points at every
offset: this sweep is minutes of the board's time and is worth
narrowing to the offsets the ruler placed the burst at.
...
class Point:
    ...
    def __init__(self, tap, row, strobes, lane=0):
        self.tap = tap
        self.row = row
        self.strobes = [strobe[lane] for strobe in strobes]
        self.lane = lane
...
class Taps:
    ...
    # several do.
    STROBE_READS = 5
    # The slots the burst and the strobe are expected in, and a few
    # either side.  Centred on the record's read offset until the ruler
    # has placed the burst on this board, which is what these want
    # re-centring on.
    OFFSETS = tuple(range(max(Walk.OFFSET - 5, 0),
                          min(Walk.OFFSET + 5, Walk.OFFSET_COUNT)))
    HOME = Walk.OFFSET
    VALUES = (0x40, 0x10)
    # Which lane's strobe the crossings are read from.  Each lane has
    # its own pair and its own routing, so a distance measured here
    ...
    async def read_point(self, tap):
        ...
        for _ in range(self.STROBE_READS - 1):
            await asyncio.sleep(self.SETTLE_SECONDS)
            strobes.append(await Walk.strobes(self.panel, "strobe_word"))

        return Point(tap, row, strobes, self.LANE)
    ...
    def table(self, value, points):
        ...
        print(f"--- poke value {value:#04x}, capture announced on the write,"
              f" strobe of lane {self.LANE}")
        print(f" tap  data at {self.HOME}"
              + " " * (3 * Walk.BEAT_BYTES - 9)
              + f"strobe at {self.HOME}, {self.STROBE_READS} passes"
              f"              at {near}             at {far}")
        for point in points:
            print(f" {point.tap:>3}  {self.spaced(point.data(self.HOME))}"
                  f"   {self.strobes(point):<44}"
                  f"  {self.compact(point.data(near))}"
                  f"  {self.compact(point.data(far))}")
        print()
