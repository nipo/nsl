...
uniformly wrong.  A cycle here is sixteen bytes -- eight slots of two
byte lanes, byte 2s + l being lane l of slot s -- so both lanes are
under the same reading and a lane that never lands stands out as every
other byte.  Changing that value and watching the answer follow
...
class Poke:
    ...
    async def answer(self):
        return Walk.burst_bytes(await Walk.wide(self.panel, "mpr"))
    ...
    async def table(self, tdqs):
        ...
        print("value   read back"
              + " " * (3 * Walk.BEAT_BYTES - 9) + "answers")
        ...
        for value in (0x10, 0x40, 0xa0):
            ...
            mark = ("  <- the whole cycle" if hits == Walk.BEAT_BYTES
                    else f"  {hits}/{Walk.BEAT_BYTES}")
            print(f" {value:#04x}   "
