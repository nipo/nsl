...
class Level:
    ...
    # The part drives every pin of a lane alike, so a lane whose eight
    # bits disagree is a bus nobody is driving; the two lanes may
    # legitimately differ from each other, since each has its own
    # strobe.
    LANE_IDLE = (0x00, 0xff)
    ...
    @classmethod
    def spread(cls, answer):
        """Which lanes answered with pins that do not agree."""
        bad = [lane for lane in range(Walk.LANES)
               if ((answer >> (8 * lane)) & 0xff) not in cls.LANE_IDLE]
        if not bad:
            return ""

        return "  (pins disagree on lane " + ", ".join(map(str, bad)) + ")"
    ...
    async def run(self):
        ...
        for odt in (1, 0):
            ...
            print(f"  not levelling:  {idle:#06x}")
            ...
            print(f"  levelling:      {first:#06x}{moving}"
                  f"{self.spread(first)}")

            high = await Walk.strobes(self.panel, "strobe_high")
            low = await Walk.strobes(self.panel, "strobe_low")
            for lane in range(Walk.LANES):
                print(f"  strobe {lane} on the pin:"
                      f" ever high {high[lane]:#04x},"
                      f" ever low {low[lane]:#04x}")
                if high[lane] and low[lane]:
            ...
            if first == idle:
                ...
                print(f"  the answer sits at {first:#06x}: the part has taken"
