...
The four outputs run at 12 MHz and are sampled, raw, by a 48.6 MHz
clock off another divider of the same PLL.  The two rates are 81 over
20 in lowest terms, so the sampling instant lands on 81 distinct
points of the 12 MHz cycle before repeating: an edge resolves to an
81st of a cycle, a little over four degrees.
...
class PhaseCheck:
    ...
    # them: 972/20 MHz over 972/81 MHz.
    SAMPLE_HZ = 48_600_000
    ...
    # nanosecond or so, which at 12 MHz is a few degrees, and the
    # grid itself is four.
    TOLERANCE_DEG = 12.0
