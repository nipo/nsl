...
begin
  ...
  -- The button is not in the reset path: it reads high with nothing
  -- pressed on this board, which would hold the clock managers in
  -- reset for good.  It stays on the panel, where it is an
  ...
  -- Every output is measured in a domain of its own, so every one of
  -- them rides a global buffer.  The four shifted ones are also
  -- sampled as plain wires, and that copy is taken ahead of the
  -- buffer: a Spartan-6 global buffer is not allowed to drive a data
  -- pin, and the tools stop rather than warn.  The four probe routes
  -- are ordinary interconnect and no longer matched to each other,
  -- which is a nanosecond or so of skew against a quarter cycle of
  -- twenty.
  ...
  -- The rack rides the crystal rather than anything the clock
  -- managers make: an instrument that only answers when its subject
  -- works tells you nothing when the subject does not.  A clock that
  -- never starts reads zero here, which is the measurement.
  observer: gatecap_generated.pll_check.pll_check_core
    ...
    port map(
      ...
      rates_dcm_36m_i => rack_s,
      rates_sample_i => phase_s(4),
      rates_f81m_i => phase_s(5),

      panel_clock_i => board_s,
      panel_reset_n_i => startup_reset_n_s,
      panel_user_btn_i => user_btn,
      panel_startup_reset_n_i => startup_reset_n_s,
      panel_ref_locked_i => ref_locked_s,
      panel_phase_locked_i => phase_locked_s,

      phases_sample_clock_i => phase_s(4),
      phases_sample_reset_n_i => sample_reset_n_s,
      phases_sample_p0_i => phase_raw_s(0),
      phases_sample_p90_i => phase_raw_s(1),
      phases_sample_p180_i => phase_raw_s(2),
      phases_sample_p270_i => phase_raw_s(3)
      );
