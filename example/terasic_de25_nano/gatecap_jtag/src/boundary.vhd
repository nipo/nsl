...
-- JTAG gatecap bench for a DE25-Nano.
--
-- The rack is reached through the part's own TAP, over the on-board
-- USB Blaster 3 the part is configured through, as a virtual JTAG
-- node behind the SLD hub Quartus inserts.  The hub finds the TAP by
-- itself, so no JTAG pin reaches this top level.
--
-- One clock and no PLL: the 50 MHz oscillator on CLOCK1_50 drives the
...
begin
  ...
  rack: gatecap_generated.de25_nano_jtag.de25_nano_jtag_core
    generic map(
      burst_length_l2_c => 6
      )
    port map(
