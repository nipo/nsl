...
class Eye:
    ...
    async def answer(self):
        """The eight bytes of the last beat the sequencer caught."""
    ...
    async def poke_map(self):
        table = Table("poke", "a burst poked straight at the DFI",
                      f"bad beats of {self.BEATS} against"
                      f" {self.POKE_VALUE:#04x} counting up, one poke a cell",
                      self.DATA_OFFSETS, range(Line.TAP_COUNT))
    ...
    async def mpr_map(self):
        table = Table("mpr",
                      ...
                      f"bad beats of {self.BEATS} against the alternating"
