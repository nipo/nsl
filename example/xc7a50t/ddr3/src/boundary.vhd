...
-- Walks an AS4C128M8D3LB through the serialiser DDR3 PHY.
--
-- The whole stack is here rather than the AXI wrapper because this PHY
-- wants four clocks, which the wrapper does not carry.
--
-- The numbers the PHY needs are board_c below: measured once with the
-- instruments this panel exposes, and applied by the PHY itself at
-- reset.  The panel keeps a copy of each of them so that an instrument
-- can still move one and watch what happens, behind a manual bit that
-- says whose numbers the PHY is to use -- because a control register
-- comes out of reset at zero, and zero is a legal value for most of
-- them.  With the bit low the design is what a design would be.
--
-- What the panel exposes besides those is the raw first word that
-- comes back, which is the thing that would have shortened every
-- memory bring-up so far: a part that is not driving the bus and a
-- part that is driving it at the wrong moment look the same through a
-- pass-or-fail line, and different the moment the word itself is
-- readable.
entity boundary is
  port(
    clk_25_i: in std_ulogic;

    ddr3_ck_p: out std_ulogic_vector(0 downto 0);
    ddr3_ck_n: out std_ulogic_vector(0 downto 0);
    ddr3_cke: out std_ulogic_vector(0 downto 0);
    ddr3_cs_n: out std_ulogic_vector(0 downto 0);
    ddr3_ras_n: out std_ulogic;
    ddr3_cas_n: out std_ulogic;
    ddr3_we_n: out std_ulogic;
    ddr3_reset_n: out std_ulogic;
    ddr3_odt: out std_ulogic_vector(0 downto 0);
    ddr3_ba: out std_ulogic_vector(2 downto 0);
    ddr3_addr: out std_ulogic_vector(13 downto 0);
    ddr3_dm: out std_ulogic_vector(0 downto 0);
...
architecture beh of boundary is
  ...
  constant board_hz_c: natural := 25000000;
  ...
  -- What the family's delay lines were told to expect
  constant delay_ref_hz_c: natural := 200000000;

  -- One block makes all four: 25 MHz over a 800 MHz oscillator divides
  -- into every one of them exactly, and a quarter of a 400 MHz cycle
  -- is four eighths of a 800 MHz one, well inside what the block
  -- shifts by.
  ...
  -- panel below carries.  Every figure but the tap is a whole slot or
  -- a whole word and belongs to the family's serialisers; the tap is
  -- sub-slot and is the one that would want checking on another unit.
  --
  ...
  -- The whole part: eight banks of sixteen thousand rows of a
  -- kilobyte.
  constant region_byte_l2_c: natural := 27;
  -- Enough to cross a burst boundary and small enough that a point of
  -- the training sweep costs nothing.
  constant probe_byte_l2_c: natural := 7;
  -- Two sizes between the two, read off the core's address map rather
  -- than picked: an access carries eight bytes, address bits 3 to 9
  -- are the column, 10 to 12 the bank and 13 up the row.  A page is
  -- therefore a kilobyte, the bank turns over every kilobyte and the
  -- row every eight of them -- so a contiguous region cannot reach a
  -- second row without having crossed every bank boundary first, and
  -- the ladder between the probe and the whole part is banks, then
  -- banks and rows.
  --
  -- Eight kilobytes is every bank at row zero: seven bank crossings,
  -- no row change, every row left open and nothing precharged.  It is
  -- also the first size that asks for four activates inside a window.
  constant bank_byte_l2_c: natural := 13;
  -- Sixteen kilobytes adds the first row crossing, which is the first
  -- precharge and reactivate a walk meets, and runs long enough to
  -- meet refreshes.
  constant row_byte_l2_c: natural := 14;

  signal board_s: std_ulogic;
  signal raw_s, buffered_s: std_ulogic_vector(0 to ram_config_c.output_count - 1);
  signal ram_s, fast_s, data_s, delay_ref_s: std_ulogic;
  ...
  signal dqs_out_s: nsl_io.io.tristated_vector(lane_c - 1 downto 0);
  signal dqs_n_unused_s: nsl_io.io.tristated_vector(lane_c - 1 downto 0);
  signal ck_pad_s: nsl_io.diff.diff_pair;
  signal dqs_in_s, dqs_delayed_s: std_ulogic_vector(lane_c - 1 downto 0);
  signal dqs_raw_s, dqs_caught_s: std_ulogic_vector(0 to 7);
  signal strobe_high_s, strobe_low_s: std_ulogic_vector(0 to 7);
  ...
  -- offset points at.
  signal dqs_window_s: std_ulogic_vector(0 to 15);
  signal strobe_pick_s, strobe_word_s: std_ulogic_vector(0 to 7);
  ...
  -- zero -- the earliest on the wire -- in the low byte, for the
  -- capture instrument to record.  The write side is taken on the PHY
  -- side of the sequencer mux, so a burst the sequencer laid down is
  -- caught alongside the ones the core does.
  signal cap_wrdata_s, cap_rddata_s:
    std_ulogic_vector(beat_byte_c * 8 - 1 downto 0);
  -- A pass that has gone wrong, and a beat the walker took off the
  -- read channel.  Both are the selected walker's, as everything else
...
begin
  ...
  buffers: for i in raw_s'range
  generate
    line: nsl_clocking.distribution.clock_buffer
      port map(
        clock_i => raw_s(i),
        clock_o => buffered_s(i)
        );
  end generate;
  ...
  probe_reset_n_s <= bus_reset_n_s when full_s = 0 else '0';
  walk_reset_n_s <= bus_reset_n_s when full_s = 1 else '0';
  bank_reset_n_s <= bus_reset_n_s when full_s = 2 else '0';
  row_reset_n_s <= bus_reset_n_s when full_s = 3 else '0';
  ...
  axi_s.m <= walk_axi_s when full_s = 1
             else bank_axi_s when full_s = 2
             else row_axi_s when full_s = 3
             else probe_axi_s;
  ...
  core: nsl_ext_ram.dram.dram_core
    generic map(
      ...
      -- Both ends of the column pipeline run at tCCD on this board.
      -- The PHY lays write bursts back to back with no preamble
      -- between them, and it reads the read announcement as DFI states
      -- it, a cycle of level per cycle of answer, so a read may be
      -- announced every cycle.  This board's round trip is nine
      -- cycles: eleven leaves it two of slack.  The PHY's read offset
      -- port reaches further than that, and a sweep that goes past
      -- what this covers reads the pins through the bench's own
      -- capture rather than through the core.
      read_ahead_c => 11,
      ...
      if level_state_s = LEVEL_OFF then
        ...
        loop
          mpr_data_s(i * 8 + 7 downto i * 8)
            <= std_ulogic_vector(dfi_slave_s.rddata(i).value(0));
        end loop;
        -- The same eight slots, on the strobe pin rather than on the
  ...
  begin
    case level_state_s is
      ...
      when LEVEL_PULSE =>
        ...
        -- part is waiting to sample the clock on.  Asked for on every
        -- cycle by default rather than now and then: a strobe that
        -- runs without a break is one a pin can be watched for, and
        -- the part samples the clock on every rising edge of it
        -- either way.  The panel can ask for neither, which is what
        -- the watcher is read against.
        -- One cycle in every level_period_c by default, which is the
        -- discrete strobe burst levelling asks for; the panel can ask
        -- for one on every cycle instead, or for none at all, which
        -- is what the pin watcher is read against.
        if force_s = "11"
  ...
  phy: nsl_ext_ram.ddr3_io.ddr3_phy_serdes
    ...
    port map(
      ...
      fast_clock_i => fast_s,
      data_clock_i => data_s,
      -- A 7-series pin drives from one clock pair and listens on
      -- another, so the data pins take the shift and there is no second
      -- parallel clock to make: what crosses to this one crosses
      -- inside the controller domain.
      shifted_clock_i => ram_s,
      delay_ref_clock_i => delay_ref_s,
      ...
      odt_o => open,
      ram_reset_n_o => ddr3_reset_n,

      dm_o => dm_s,
      dqs_o => dqs_out_s,
      -- The pad makes its own complement, so what the PHY serialises
      -- alongside goes nowhere.  It cannot be left open: the formal is
      -- unconstrained and takes its width from what is bound to it.
      dqs_n_o => dqs_n_unused_s,
      dq_o => dq_out_s,
      dq_i => dq_in_s
      );

  ddr3_ba <= std_ulogic_vector(ram_ba_s);
  ddr3_addr <= std_ulogic_vector(ram_a_s);
  ddr3_dm <= dm_s;
  ddr3_odt(0) <= odt_s;

  -- Both halves of a pair off one buffer.  Driving them as two
  -- ordinary pins moves the crossing point by whatever their routing
  -- differs by, and the part reads its clock and its strobe from that
  -- crossing.
  clock_pad: nsl_io.pad.pad_diff_output
    generic map(
      is_clock => true
      )
    port map(
  ...
  ddr3_ck_p(0) <= ck_pad_s.p;
  ddr3_ck_n(0) <= ck_pad_s.n;
  ...
  generate
    ...
    -- same ticks.  Two reasons: the family's line adds most of a
    -- nanosecond even at its shortest tap, so a pin that skips it is
    -- sampled the better part of a slot ahead of one that does not;
    -- and the strobe runs the pattern CK runs, off the clock the
    -- capture samples on, so its edges land on the sampling edge and
    -- raw it would be caught on its own transitions.
    --
    ...
    --
    -- The PHY places its own lines at the board record's tap out of
    -- reset and this one is not in that group, so out of reset the two
    -- sit read_tap apart.  Every instrument that reads the strobe
    -- walks the whole group to the mark first and then out to the tap
    -- it wants, which puts them back together.
    ...
    listener: nsl_io.serdes.serdes_input
      generic map(left_first_c => true, ddr_mode_c => true,
                  from_delay_c => true,
                  ratio_c => 8)
      port map(serial_clock_i => fast_s,
               parallel_clock_i => ram_s, reset_n_i => ram_reset_n_s,
               serial_i => dqs_delayed_s(i),
               parallel_o => dqs_raw_s,
               bitslip_i => '0',
               mark_o => open);

    -- Back to the sense the schedule holds, so the ruler and the drive
    -- map read the strobe as the PHY means it whichever way the panel
    -- turns the pattern round.  Taken here and not at the pin because
    -- the delay line's input comes straight from the pad primitive and
    -- nothing may sit between them.
    sense: for s in 0 to 7
    generate
      dqs_caught_s(s) <= dqs_raw_s(s) xor phy_invert_s;
    end generate;
  end generate;

  -- The strobe pin read out with the arithmetic the PHY reads the data
  -- pins out with, pipelined to land on the same cycle.
  --
  ...
  begin
    if ram_reset_n_s = '0' then
      dqs_window_s <= (others => '0');
      strobe_pick_s <= (others => '0');
    elsif rising_edge(ram_s) then
      dqs_window_s(0 to 7) <= dqs_window_s(8 to 15);
      dqs_window_s(8 to 15) <= dqs_caught_s;

      slot_sel := to_integer(phy_offset_s(2 downto 0));
      for s in 0 to 7
      loop
        strobe_pick_s(s) <= dqs_window_s(slot_sel + s);
      end loop;
    end if;
  end process;

  -- Sticky over a levelling pass or a walk: a slot that was ever high
  -- and a slot that was ever low.  A strobe that toggles makes the two differ; a
  -- pin nobody drives makes them agree, whatever level it settles at.
  strobe_watch: process(ram_s, ram_reset_n_s) is
  begin
    if ram_reset_n_s = '0' then
      strobe_high_s <= (others => '0');
      strobe_low_s <= (others => '0');
    elsif rising_edge(ram_s) then
  ...
  -- Whichever of the two walkers the panel selected is the one that
  -- answers; the other is held in reset and has nothing to say.
  busy_s <= walk_busy_s when full_s = 1
            else bank_busy_s when full_s = 2
            else row_busy_s when full_s = 3
            else probe_busy_s;
  done_s <= walk_done_s when full_s = 1
            else bank_done_s when full_s = 2
            else row_done_s when full_s = 3
            else probe_done_s;
  error_count_s <= walk_errors_s when full_s = 1
                   else bank_errors_s when full_s = 2
                   else row_errors_s when full_s = 3
                   else probe_errors_s;
  first_error_s <= walk_first_s when full_s = 1
                   else bank_first_s when full_s = 2
                   else row_first_s when full_s = 3
                   else probe_first_s;
  ...
  -- What the capture records of the data bus.  The DFI carries a cycle
  -- as an array of slots and the host reads a flat word, so the two
  -- are laid out here rather than in the instrument: slot zero is the
  -- first beat out of the part and the first byte of the word.
  cap_data: process(dfi_phy_s, dfi_slave_s) is
  ...
  cap_error_s <= '0' when error_count_s = 0 else '1';

  -- Where a pass begins: the run, with a controller that has come
  ...
  -- window a thousand samples deep.  Ready rises on the cycle the
  -- core can take a transaction, which is the cycle the walker has
  -- been waiting for since it left reset.
  cap_run_s <= run_s and ready_s;
  cap_r_beat_s <= '1' when is_valid(axi_config_c, axi_s.s.r)
                  and is_ready(axi_config_c, axi_s.m.r) else '0';

  observer: gatecap_generated.ddr3_walk.ddr3_walk_core
    generic map(
      burst_length_l2_c => 6
      )
    port map(
      ...
      panel_first_read_low_i => unsigned(first_read_s(31 downto 0)),
      panel_first_read_high_i => unsigned(first_read_s(63 downto 32)),
      panel_read_count_i => read_count_s,
      panel_delay_mark_i => unsigned(delay_mark_s),
      panel_level_answer_i => unsigned(level_answer_s),
      panel_strobe_high_i => unsigned(strobe_high_s),
      panel_strobe_low_i => unsigned(strobe_low_s),
      panel_level_step_i => level_step_s,
      panel_level_burst_i => level_burst_s,
      panel_mpr_low_i => unsigned(mpr_data_s(31 downto 0)),
      panel_mpr_high_i => unsigned(mpr_data_s(63 downto 32)),
      panel_mpr_count_i => mpr_count_s,
      panel_strobe_word_i => unsigned(strobe_word_s),

      capture_ram_clock_i => ram_s,
      capture_ram_reset_n_i => ram_reset_n_s,
      capture_ram_run_i => cap_run_s,
      capture_ram_error_i => cap_error_s,
      capture_ram_cs_n_i => dfi_phy_s.command(0).cs_n,
      capture_ram_ras_n_i => dfi_phy_s.command(0).ras_n,
      capture_ram_cas_n_i => dfi_phy_s.command(0).cas_n,
      capture_ram_we_n_i => dfi_phy_s.command(0).we_n,
      capture_ram_bank_i =>
        std_ulogic_vector(dfi_phy_s.command(0).bank(2 downto 0)),
      capture_ram_address_i =>
        std_ulogic_vector(dfi_phy_s.command(0).address(13 downto 0)),
      capture_ram_wrdata_en_i => dfi_phy_s.wrdata_en(0),
      capture_ram_wrdata_i => cap_wrdata_s,
      capture_ram_rddata_valid_i => dfi_slave_s.rddata_valid,
      capture_ram_rddata_i => cap_rddata_s,
      capture_ram_error_count_i => std_ulogic_vector(error_count_s(7 downto 0)),
