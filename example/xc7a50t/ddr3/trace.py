...
class Burst:
    ...
    def rotation_of(self, other):
        """By how many beats this burst is the other one turned round.

        The two directions are one rotation each: a burst that came
        back a beat late and one that came back a beat early differ
    ...
    def slide_of(self, other):
        """By how many beats this burst is the other one moved along,
        without the beats that fell off the end coming round.

        A burst sampled a beat late holds seven of the write's beats
        and one beat of whatever the bus held; a rotation would have
        put the eighth back at the other end.
        """
        mine, theirs = self.beats, other.beats
        for shift in range(1, self.BEATS):
...
class Trace:
    DEPTH = 1024
    # The read path goes where the board record puts it rather than
    # where a copy of it here does: a script that carries its own
    # numbers reads one word late the day the record moves.
    OFFSET = Walk.OFFSET
    TAP = Walk.TAP
    PASSES = 40
    POLL_SECONDS = 0.05
    ...
    def classify(read, written, earlier):
        ...
        if rotation is not None:
            return f"shifted by {rotation} beats"

        slide = read.slide_of(written)
        if slide is not None:
            return f"shifted by {abs(slide)} beats"
