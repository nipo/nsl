...
What to expect, from the read reference alone: the predefined pattern
comes back at read offset 44 with the part's CAS latency of six, so a
burst launched a CAS write latency of five after its command, seen in
loopback with no flight either way, belongs near 44 - 2*6 + 2*5 = 42,
give or take a slot.  A whole cycle away from that -- eight slots --
...
class Ruler:
    ...
    def report(self, value, rows):
        ...
        print("offset  data, slot 0 first               strobe")
            ...
            for near in (offset - 1, offset, offset + 1):
                if 0 <= near < len(rows):
                    ...
                    print(f"    strobe at offset {near}:"
                          f" {rows[near][1]:08b}{mark}")

        toggling = [offset for offset, (_, strobe) in enumerate(rows)
                    if f"{strobe:08b}" in TOGGLING]
        if not toggling:
            print("  the strobe toggles at no offset: over the slots this"
                  " capture covers it never moves")
        else:
            print("  the strobe toggles at these offsets, and the data"
                  " beside it:")
