...
class Reading:
    ...
    def strobe_passes(self, slot):
        offset, place = Window.source(slot)
        return [(strobe >> (7 - place)) & 1
                for strobe in self.strobes[offset]]

    def data_passes(self, slot):
        offset, place = Window.source(slot)
        return [(word >> (8 * place)) & 0xff for word in self.words[offset]]

    def strobe_map(self):
        """One character a slot: the level it held, or that it held none."""
        marks = []
        for slot in Window.slots():
            bit = self.steady(self.strobe_passes(slot))
    ...
    def data_map(self):
        ...
        cells = []
        for slot in Window.slots():
            byte = self.steady(self.data_passes(slot))
...
class DriveMap:
    ...
    async def capture(self):
        """One pass: the eight captured bytes as a word, and the strobe.

        Taken in one burst of the status region, so the data word and
        the strobe word are the same capture rather than two.
        """
        status = (await self.panel.snapshot())["status"]
        word = (status["mpr_high"] << 32) | status["mpr_low"]
        return word, status["strobe_word"]
    ...
    def report(self, odt, reading):
        ...
        marks = reading.strobe_map()
        cells = reading.data_map()
        half = (Window.LAST + 1 - Window.FIRST) // 2
        print(f"  strobe slots {Window.FIRST}..{Window.LAST}:  {marks}")
        print(f"  data   slots {Window.FIRST}..{Window.FIRST + half - 1}: "
              " " + " ".join(cells[:half]))
        print(f"  data   slots {Window.FIRST + half}..{Window.LAST}: "
              " " + " ".join(cells[half:]))

        self.extents("strobe", marks,
                     lambda mark: mark != Reading.UNSTEADY,
                     lambda mark: mark == Reading.UNSTEADY)
        self.extents("data", cells,
