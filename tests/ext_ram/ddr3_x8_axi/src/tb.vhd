...
-- Walks a region of an x8 DDR3 part through the controller's AXI port,
-- once in address order and once jumping across rows and banks.
--
-- Same chain as the x16 bench beside it, against the narrowest bus
-- DDR3 comes in: one byte lane, one strobe pair, and a page of a
-- kilobyte instead of two.  A burst of eight is eight bytes here
-- rather than sixteen, so the beat, the AXI data bus and the column
-- address all move, and a controller that had folded the lane count
-- into an assumption shows it here.
entity tb is
end entity;
...
architecture beh of tb is
  ...
  -- DDR3-800, which is the rate the board this part sits on is meant
  -- to run at.  The part is a faster grade and honours the same
  -- delays.
