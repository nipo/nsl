...
-- Walks an x16 DDR3 part in address order over a region wide enough to
-- leave the open row and the open bank several times, and measures
-- what a burst costs.
--
-- Same walk as the x8 bench beside it, against the other geometry the
-- controller is used with: two byte lanes, a page of two kilobytes
-- instead of one, and fifteen row bits instead of fourteen.  A burst
-- of eight beats carries sixteen bytes a beat here rather than eight,
-- so it spans the same eight part bursts and the same eight columns
-- while covering twice the address space.  The page and the row move
-- one bit up, and a controller that had folded the lane count into its
-- bank or refresh cadence shows it here.
--
...
-- differs from these two only in how long it takes.  The bench checks
-- that the top pass really ends on the last row of the part, because a
-- row bit that never leaves zero is exactly the fault this walk is
-- meant to catch.
--
...
architecture beh of tb is
  ...
  -- DDR3-800, the slowest bin the part supports and the rate the board
  -- this part sits on runs at.
  ...
  -- What the region spans, in the map's own terms: pages, which are
  -- also bank changes here, and rows.
  constant region_page_count_c: natural := 2 ** (region_byte_l2_c - layout_c.bank_low);
  constant region_row_count_c: natural := 2 ** (region_byte_l2_c - layout_c.row_low);

  constant pass_count_c: natural := 2;

  constant top_row_c: unsigned(layout_c.row_width - 1 downto 0) := (others => '1');
...
begin
        ...
        if is_valid(axi_config_c, axi_s.m.ar)
          ...
          if ar_count = 0 then
            ...
            assert refresh_count_s - refresh_at_start >= 2
              ...
              severity failure;
            assert activate_count_s - activate_at_start >= region_page_count_c
              report "write pass does not open every page of the region"
              severity failure;
          end if;
          ar_count := ar_count + 1;
        end if;
  ...
  stim: process is
    variable last_row: unsigned(layout_c.row_width - 1 downto 0);
  begin
    ...
    assert region_page_count_c >= 2
      report "walk does not cross a bank boundary"
      severity failure;

    -- The top pass has to end on the last row of the part, otherwise
    -- the high row bits never leave zero in either pass.
    last_row := row_of(layout_c,
                       to_unsigned(pass_base(pass_count_c - 1)
                                   + 2 ** region_byte_l2_c - 1,
                                   axi_config_c.address_width));
    assert last_row = top_row_c
      report "top pass does not reach the last row of the part"
      severity failure;
    ...
    report "walking " & integer'image(2 ** region_byte_l2_c) & " bytes in "
      ...
      & integer'image(burst_byte_c) & " bytes, over "
      & integer'image(region_row_count_c) & " rows and "
      & integer'image(region_page_count_c) & " pages";
