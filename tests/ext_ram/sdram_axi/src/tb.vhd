...
-- Walks a region of a single data rate SDRAM through the controller's
-- AXI port, once in address order and once jumping across rows and
-- banks.  The first pass leans on rows staying open between accesses,
-- the second makes almost every access reopen one.
--
-- The part model checks the command stream underneath, so a pass
-- covers the data path and the timings alike.
entity tb is
end entity;
