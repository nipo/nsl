...
entity terminal_frame_generator is
  generic(
...
begin
  ...
  sequencer: block is
    ...
    signal cell_packed_s, cell_read_packed_s: cell_packed_t;
    signal scan_glyph_line_s: glyph_line_index_t;
    signal cell_read_address_s: cell_address_t;
  begin
    scanner: block is
      type state_t is (
        ST_WAIT_SOF,
    ...
    begin        
      ...
      begin
        line_reset_n_s <= '1';
        scan_address_s <= to_unsigned(r.row, row_count_l2_c)
    ...
    streamer: nsl_memory.streamer.memory_streamer
      generic map(
        addr_width_c => cell_address_t'length,
        data_width_c => cell_packed_t'length,
        memory_latency_c => cell_latency_c,
      ...
      port map(
        ...
        mem_enable_o => cell_enable_o,
        mem_address_o => cell_read_address_s,
        mem_data_i => cell_read_packed_s
        );

    cell_row_o <= cell_read_address_s(cell_address_t'left downto column_count_l2_c);
    cell_column_o <= cell_read_address_s(column_count_l2_c-1 downto 0);
    cell_read_packed_s <= cell_pack(cell_t'(
      char => cell_character_i,
      fg => cell_foreground_i,
      bg => cell_background_i,
      underline => cell_underline_i
      ));
  ...
  renderer: block is
    ...
    signal r, rin: regs_t;
  begin        
    regs: process(clock_i, line_reset_n_s) is
    begin
      if rising_edge(clock_i) then
        r <= rin;
      end if;
