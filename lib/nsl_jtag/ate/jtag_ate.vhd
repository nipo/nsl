...
entity jtag_ate is
  ...
  port (
    ...
    tdo_delay_i : in unsigned(tdo_delay_max_l2_c-1 downto 0) := (others => '0');
...
architecture rtl of jtag_ate is
  ...
  signal s_delayed_sample_tick: std_ulogic;
  signal s_sample_tick: std_ulogic;
...
begin
  ...
  transition_tdi: process(cmd_data_i, cmd_op_i, cmd_size_m1_i, cmd_valid_i,
                          ...
                          s_delayed_sample_tick, s_rsp_valid, tick_i) is
  begin
    ...
    case r.state_out is
      ...
      when ST_SHIFT_LOW =>
        if s_delayed_sample_tick = '1' then
  ...
  no_delay: if tdo_delay_max_l2_c = 0
  generate
    s_delayed_sample_tick <= s_sample_tick;
  end generate;

  has_delay: if tdo_delay_max_l2_c /= 0
  generate
    tick_variable_delay: nsl_event.tick.tick_variable_delay
      generic map (
        delay_max_l2_c => tdo_delay_max_l2_c
        )
      port map (
        clock_i   => clock_i,
        reset_n_i => s_tick_n_rst,
        tick_i    => s_sample_tick,
        tick_o    => s_delayed_sample_tick,
        delay_i   => tdo_delay_i
        );
  end generate;
